describe('Create Point Settings Negative - Validation Konversi Point', () => {
    beforeEach(() => {
        cy.login();
    ...
    it('kolom Konversi Point hanya menerima angka dan tidak boleh 0 atau negatif', () => {
        ...
        cy.get('input[id*="Konversi Point"]')
            .as('konversiPointInput');

        cy.get('@konversiPointInput').clear().type('abc');
        assertInvalidCreateSubmission();

        cy.get('@konversiPointInput').clear().type('0');
        assertInvalidCreateSubmission();
